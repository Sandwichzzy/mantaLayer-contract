...
contract RewardManager is Initializable, OwnableUpgradeable, ReentrancyGuard, RewardManagerStorage {
    ...
    constructor(IDelegationManager _delegationManager, IStrategyManager _stragegyManager, IERC20 _rewardTokenAddress) {
        i_delegationManager = _delegationManager;
        i_strategyManager = _stragegyManager;
        i_rewardTokenAddress = _rewardTokenAddress;
        _disableInitializers();
    }

    function initialize(address initialOwner, address _rewardManager, address _payFeeManager, uint256 _stakePercent)
        external
        initializer
    {
    ...
    function payFee(address strategy, address operator, uint256 baseFee) external onlyPayFeeManager {
        ...
        uint256 operatorShares = i_delegationManager.operatorShares(operator, IStrategyBase(strategy));
        require(
        ...
        emit OperatorAndStakeReward(strategy, operator, stakeFee, operatorFee);
    }

    function operatorClaimReward() external returns (bool) {
        uint256 claimAmount = operatorRewards[msg.sender];
        require(claimAmount > 0, "RewardManager.operatorClaimReward: no reward to claim");

        require(
            i_rewardTokenAddress.balanceOf(address(this)) >= claimAmount,
            "RewardManager.operatorClaimReward:  Reward Token balance insufficient"
        );

        operatorRewards[msg.sender] = 0;
        i_rewardTokenAddress.safeTransfer(msg.sender, claimAmount);
    ...
    function stakeHolderClaimReward(address strategy) external returns (bool) {
        ...
        require(
            i_rewardTokenAddress.balanceOf(address(this)) >= stakeHolderAmount,
            "RewardManager operatorClaimReward: Reward Token balance insufficient"
        );
        strategyStakeRewards[strategy] -= stakeHolderAmount;
        i_rewardTokenAddress.safeTransfer(msg.sender, stakeHolderAmount);
        emit StakeHolderClaimReward(msg.sender, strategy, stakeHolderAmount);
        return true;
    }

    function getStakeHolderAmount(address strategy) external view returns (uint256) {
        return _stakeHolderAmount(msg.sender, strategy);
    }

    function _stakeHolderAmount(address staker, address strategy) internal view returns (uint256) {
        uint256 stakeHoldersShare = i_strategyManager.stakerStrategyShares(staker, IStrategyBase(strategy));
        uint256 strategyTotalShares = IStrategyBase(strategy).totalShares();
        if (stakeHoldersShare == 0 || strategyTotalShares == 0) {
            return 0;
        }
        return strategyStakeRewards[strategy] * stakeHoldersShare / strategyTotalShares;
    }
